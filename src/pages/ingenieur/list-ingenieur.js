...
export default function ListeIngenieur() {
    ...
    const sendRequest = async () => {
      try {
        const response = await fetch(
          `http://localhost:5000/api/ingenieur/`
        );
                ...
                {list ? (
                  list.map((row) => (
                    <TableRow key={row._id}>
                      ...
                      <Link to={`/update-ingenieur/${row._id}`}>
                        ...
                        <DeleteForeverIcon
                          ...
                          onClick={async (event) => {
                            try {
                              ...
                              setsuccess("Ingénieur bien suprimer");
                            } catch (err) {
                              console.log(err);
